...
const submitLongtermInterview = async (req, res) => {
  try {
    const { questions = [], answers = [], proctorWarnings = [], report = null } = req.body;
    ...
    if (report && typeof report === 'object') {
      savedReport = await InterviewReport.findOneAndUpdate(
        { requestId: request._id, nurseId: req.user._id },
        {
          $set: {
            candidateName: report.candidateName || req.user.name || 'Nurse Candidate',
            interviewDate: report.interviewDate ? new Date(report.interviewDate) : new Date(),
            overallScore: Number(report.overallScore || 0),
            recommendation: report.recommendation || 'Review Required',
            scores: report.scores || {},
            attentionEvents: Array.isArray(report.attentionEvents) ? report.attentionEvents : proctorWarnings,
            strengths: Array.isArray(report.strengths) ? report.strengths : [],
            concerns: Array.isArray(report.concerns) ? report.concerns : [],
            adminNotes: report.adminNotes || '',
            transcript: Array.isArray(report.transcript) ? report.transcript : [],
            patientProblem: request.problem || report.patientProblem || '',
...
${Array.isArray(proctorWarnings) && proctorWarnings.length
  ...
  : 'No warnings recorded.'}`,
    ...
    if (passed) {
      if (savedReport) {
        savedReport.backendScore = score;
        savedReport.backendPassed = true;
        savedReport.backendFeedback = parsed.feedback || '';
    ...
    if (savedReport) {
      savedReport.backendScore = score;
      savedReport.backendPassed = false;
      savedReport.backendFeedback = parsed.criticalFailReason || parsed.feedback || '';
      await savedReport.save();
    }
