...
export default function AdminDashboard() {
  ...
  async function fetchInterviewReports() {
    ...
    const localReports = loadInterviewReports();
    try {
      const res = await axios.get("/api/admin/interview-reports");
      const dbReports = (res.data?.reports || []).map(report => ({
        ...report,
        id: report._id || report.id,
        nurseId: report.nurseId?._id || report.nurseId,
        candidateName: report.candidateName || report.nurseId?.name || "Nurse Candidate",
        patientProblem: report.patientProblem || report.requestId?.problem,
      }));
      const seen = new Set(dbReports.map(report => `${report.requestId}-${report.nurseId}`));
      const merged = [
        ...dbReports,
        ...localReports.filter(report => !seen.has(`${report.requestId}-${report.nurseId}`)),
      ];
      setInterviewReports(merged);
    } catch (err) {
      console.error("Fetch interview reports failed:", err?.response?.data?.error);
      setInterviewReports(localReports);
    }
  }
