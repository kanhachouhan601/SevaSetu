...
export default function useFaceDetection({ videoRef, enabled, onAttentionEvent } = {}) {
  ...
  const movementSinceRef = useRef(null);
  const tooFarSinceRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      ...
      try {
        ...
        const centerMove = lastBox
          ? Math.hypot(centerX - lastBox.centerX, centerY - lastBox.centerY) / Math.max(videoWidth, videoHeight)
          : 0;
        const sizeChange = lastBox?.areaRatio
          ? Math.abs(faceAreaRatio - lastBox.areaRatio) / Math.max(lastBox.areaRatio, 0.001)
          : 0;
        lastBoxRef.current = { centerX, centerY, areaRatio: faceAreaRatio };

        if (faceAreaRatio < 0.035) {
          setFaceState("too_far");
          if (!tooFarSinceRef.current) tooFarSinceRef.current = now;
          if (now - tooFarSinceRef.current > 2500) {
            addEvent("too_far", "Aap camera se bahut door lag rahe hain. Interview ke dauran seat par camera ke saamne rahiye.");
          }
          return;
        }
        tooFarSinceRef.current = null;

        const unstableMovement = centerMove > 0.12 || sizeChange > 0.45;
        if (unstableMovement) {
          setFaceState("moving");
          if (!movementSinceRef.current) movementSinceRef.current = now;
          if (now - movementSinceRef.current > 1800) {
            addEvent("movement", "Please seat par stable rahiye. Interview mein zyada movement avoid karein aur camera par focus rakhein.");
          }
          return;
        }
        movementSinceRef.current = null;
